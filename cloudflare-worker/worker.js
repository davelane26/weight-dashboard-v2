...
export default {
  async fetch(request, env) {
    ...
    // ── POST /weight/patch  (single reading, merge-by-date) ─────────────
    // For Kage (Health Connect WeightRecord), which polls every ~15min and
    // must NOT blindly replace the whole array like POST /weight does --
    // that's what caused the 2026-08-23 data-loss incident. This merges one
    // reading in by its exact date string (same identity Kage will keep
    // re-sending until a genuinely new weigh-in happens, so repeat polls
    // are a safe no-op replace, not a pile of duplicates), then best-effort
    // mirrors the same merge into Weight-tracker/data.json on GitHub so git
    // stays the complete historical record regardless of which pipeline
    // (MQTT bridge, add-weighin.html, or this) produced a given reading.
    if (method === 'POST' && url.pathname === '/weight/patch') {
      if (!await isAuthorized(request, env)) return cors('{"error":"Unauthorized"}', 401);
      let body;
      try { body = await request.json(); } catch { return cors('{"error":"Invalid JSON"}', 400); }
      if (!body.date || typeof body.weight !== 'number' || body.weight <= 0) {
        return cors('{"error":"date and a positive weight are required"}', 400);
      }
      ...
      let gitSynced = false, gitError = null;
      try {
        await mirrorWeightToGitHub(merged, env);
        gitSynced = true;
      } catch (e) {
        gitError = e.message;
        console.error('weight/patch: git mirror failed:', e);
      }

      return cors(JSON.stringify({ ok: true, date: entry.date, id: entry.id, gitSynced, gitError }));
    }
...
// Mirrors mqtt_bridge.py's single-reading update logic: same date string ->
// replace in place (keeps the original id); new date -> append with the
// next id. Only the fields the caller actually sent get set -- Kage won't
// have bodyFat/muscle/water/bone/bmr/tdee (those come from the scale itself,
// not Health Connect's WeightRecord), so those stay absent rather than
// clobbering a fuller entry that might already exist for that date.
function mergeWeightEntry(stored, body) {
  const nn = (v) => { const x = Number(v); return Number.isFinite(x) ? x : undefined; };
  const entry = { date: body.date, weight: nn(body.weight) };
  for (const key of ['weight_kg', 'bmi', 'bmr', 'bodyFat', 'bone', 'muscle', 'tdee', 'water']) {
    const v = nn(body[key]);
    if (v !== undefined) entry[key] = v;
  }

  const idx = stored.findIndex((r) => r.date === body.date);
  if (idx >= 0) {
    entry.id = stored[idx].id;
    const merged = [...stored];
    merged[idx] = { ...stored[idx], ...entry };
    return { merged, entry: merged[idx] };
  }
  entry.id = (stored.reduce((m, r) => Math.max(m, r.id || 0), 0) || 0) + 1;
  const merged = [...stored, entry].sort((a, b) => (a.date > b.date ? 1 : -1));
  return { merged, entry };
}

// ── Best-effort mirror of a weight merge into Weight-tracker/data.json ──────
// Same GET-sha-then-PUT pattern add-weighin.html uses client-side, run
// server-side here so Kage (which only holds an API_SECRET, not a GitHub
// token) doesn't need direct GitHub access. Requires the WEIGHT_GITHUB_TOKEN
// secret (a PAT scoped to contents:write on davelane26/Weight-tracker) --
// if it's not set, this throws and the caller logs+reports gitSynced:false
// without failing the KV write, so the dashboard still updates either way.
async function mirrorWeightToGitHub(mergedRows, env) {
  const token = env.WEIGHT_GITHUB_TOKEN;
  if (!token) throw new Error('WEIGHT_GITHUB_TOKEN secret not set on this Worker');
  ...
  const getResp = await fetch(`${api}?ref=main`, { headers });
  if (!getResp.ok) throw new Error(`GET data.json: ${getResp.status} ${await getResp.text()}`);
  const meta = await getResp.json();

  const content = btoa(unescape(encodeURIComponent(JSON.stringify(mergedRows, null, 2))));
  const putResp = await fetch(api, {
    method: 'PUT',
    headers: { ...headers, 'Content-Type': 'application/json' },
    body: JSON.stringify({
      message: `Weight sync from Kage: ${mergedRows[mergedRows.length - 1]?.weight ?? '?'} lb`,
      content,
      sha: meta.sha,
      branch: 'main',
    }),
